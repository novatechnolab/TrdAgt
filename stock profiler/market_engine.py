...
class SymbolProfiler:
    ...
    def __init__(self, symbol: str, token: int, fo_token: int | None = None):
        self.symbol   = symbol
        self.token    = token
...
class MarketRegistry:
    ...
    def add(self, symbol: str, token: int, fo_token: int | None = None) -> SymbolProfiler:
        with self._lock:
            if token not in self._profilers:
                p = SymbolProfiler(symbol, token, fo_token)
    ...
    def list_symbols(self) -> list[dict]:
        with self._lock:
            return [{"symbol": p.symbol, "token": p.token, "fo_token": p.fo_token}
