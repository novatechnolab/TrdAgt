...
- **OI Heatmap Mobile Search Bar & Virtual Keyboard Fix:** Cleared default pre-filled `NIFTY` value so search box defaults empty with placeholder, removed `.select()` mobile focus-cancellation quirks, upgraded search input to 16px with `touch-action: auto !important;` and `pointer-events: auto;`, and configured native `inputmode="search"` for immediate mobile virtual keyboard activation.
