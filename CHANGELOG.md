...
- **OI Heatmap Mobile Search Bar & Virtual Keyboard Fix:** Removed CSS containment isolation from `.oi-hm-wrap` that prevented focus propagation on touch devices, upgraded search bar height ($\ge 42\text{px}$) and font size ($13\text{px}$), and added `inputmode="search"`, `type="search"`, `autocapitalize="characters"`, and `onclick/onfocus="this.select();"` to guarantee immediate virtual keypad activation on mobile.
