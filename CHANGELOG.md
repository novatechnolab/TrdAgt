...
- **Zero-Default OI Heatmap Screen & Mobile Virtual Keyboard Fix:** Removed default `NIFTY` auto-load so OI Heatmap screen opens clean with search bar and empty-state prompt until an F&O stock is searched or clicked from the Master Board, enabled 16px soft-keyboard direct focus on mobile touch, and preloaded stock universe in background.
